          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a a href="javascipt:;" @click="toggleCheckAll">
...
<script type="text/ecmascript-6">
    ...
    export default{
        ...
        methods:{
          ...
          delCart:function(){            //删除商品接口
            axios.post('/users/cartDel',{
              productId:this.delItem.productId
            }).then((response)=>{
              let res = response.data;
              if(res.status = '0'){
                this.modalConfirm = false; //如果删除成功则关闭模态框
                var delCount = this.delItem.productNum;  //减去的商品数
                this.$store.commit("updateCartCount",-delCount);
                this.init();
              }
            });
          },
          delCartConfirm:function(item){      //删除商品按钮
            this.delItem = item;
            this.modalConfirm = true;     //显示模态框
          },
          editCart:function(flag,item){    //编辑商品属性（个数、勾选与否）
            if(flag=='add'){
              item.productNum++;
            }else if(flag=='minu'){
