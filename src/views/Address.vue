  ...
  export default{
      data:function(){
          return{
            addressList:[],
            limit:3,
            checkIndex:0,    //单选地址，默认为第一个
            selectedAddrId:'',   //需要传递到下一个页面的数据，即地址id
            isMdShow:false,
            addressId:''      //删除选择
          }
      },
      ...
      methods:{
        init:function(){            //获取地址列表
          axios.get('users/addressList').then((response)=>{
            let res = response.data;
            this.addressList = res.result;
            this.selectedAddrId = this.addressList[0].addressId;  //获取默认地址
          })
        },
        expand:function(){                //改变more
          if(this.limit==3){
        ...
        setDefault:function(addressId){           //设置默认
        ...
        delAddressConfirm:function(addressId){       //打开对话框
          this.isMdShow = true;
          this.addressId = addressId;
        },
        delAddress:function(addressId){         //删除地址
